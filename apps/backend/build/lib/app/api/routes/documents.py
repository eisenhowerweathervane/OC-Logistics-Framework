...
_any_staff = Depends(require_roles(["owner", "dispatcher", "driver"]))


@router.post("/presign", response_model=PresignResponse)
async def presign_upload(body: PresignRequest, user: CurrentUser = _any_staff):
...
async def create_document(body: DocumentCreate, db: DbDep, user: CurrentUser = _any_staff):
