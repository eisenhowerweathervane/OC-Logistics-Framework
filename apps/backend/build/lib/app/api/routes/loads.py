...
async def create_load(body: LoadCreate, db: DbDep, user: CurrentUser = _dispatcher):
...
async def update_load(load_id: uuid.UUID, body: LoadUpdate, db: DbDep, user: CurrentUser = _dispatcher):
...
async def assign_load(load_id: uuid.UUID, body: AssignmentCreate, db: DbDep, user: CurrentUser = _dispatcher):
