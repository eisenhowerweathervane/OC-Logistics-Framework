...
async def test_dispatch_alert_no_phone(client: AsyncClient):
    ...
    driver_resp = await client.post("/api/drivers", json={
    ...
    resp = await client.post("/api/notifications/whatsapp/dispatch-alert", json={
        "driver_id": driver_id,
        "load_id": load_id,
    })
    assert resp.status_code == 200
...
async def test_docs_reminder_driver_not_found(client: AsyncClient):
    ...
    resp = await client.post("/api/notifications/whatsapp/docs-reminder", json={
