...
async def test_create_maintenance_item(client: AsyncClient):
    ...
    resp = await client.post("/api/compliance/maintenance", json={
...
async def test_list_maintenance_items(client: AsyncClient):
    ...
    await client.post("/api/compliance/maintenance", json={
        "vehicle_id": vehicle_id, "category": "tires",
    })
...
async def test_update_maintenance_status(client: AsyncClient):
    vehicle_id = await _create_vehicle(client)
    create = await client.post("/api/compliance/maintenance", json={
        "vehicle_id": vehicle_id, "category": "brake_inspection",
    })
...
async def test_create_annual_inspection(client: AsyncClient):
    vehicle_id = await _create_vehicle(client)
    resp = await client.post("/api/compliance/annual-inspections", json={
        "vehicle_id": vehicle_id,
...
async def test_list_annual_inspections(client: AsyncClient):
    ...
    await client.post("/api/compliance/annual-inspections", json={
        "vehicle_id": vehicle_id, "inspected_at": "2026-01-15",
    })
...
async def test_create_roadside_inspection(client: AsyncClient):
    vehicle_id = await _create_vehicle(client)
    resp = await client.post("/api/compliance/roadside-inspections", json={
        "vehicle_id": vehicle_id,
...
async def test_create_fuel_purchase(client: AsyncClient):
    vehicle_id = await _create_vehicle(client)
    resp = await client.post("/api/fuel/purchases", json={
        "vehicle_id": vehicle_id,
...
async def test_create_eld_day(client: AsyncClient):
    ...
    resp = await client.post("/api/eld/days", json={
        "driver_id": driver_id,
        "vehicle_id": vehicle_id,
...
async def test_list_eld_days(client: AsyncClient):
    vehicle_id = await _create_vehicle(client)
    driver_id = await _create_driver(client)
    await client.post("/api/eld/days", json={
        "driver_id": driver_id,
        "vehicle_id": vehicle_id,
        "date_local": "2026-03-10",
