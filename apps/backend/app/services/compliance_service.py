...
async def calculate_ifta_fuel(db: AsyncSession, org_id: uuid.UUID, year: int, quarter: int) -> list[dict]:
    ...
    return [{"jurisdiction": row.jurisdiction, "gallons": float(row.total_gallons)} for row in result.all()]


async def calculate_ifta_miles(db: AsyncSession, org_id: uuid.UUID, year: int, quarter: int) -> list[dict]:
    """
    ...
    return [{"jurisdiction": st, "miles": round(miles, 2)} for st, miles in sorted(miles_by_state.items())]


async def build_ifta_return(db: AsyncSession, org_id: uuid.UUID, year: int, quarter: int) -> IftaReturn:
    """
    ...
    existing_dist = await db.execute(
        select(IftaDistanceByJurisdiction).where(IftaDistanceByJurisdiction.ifta_return_id == ifta_return.id)
    )
    for row in existing_dist.scalars().all():
        await db.delete(row)

    existing_fuel = await db.execute(
        select(IftaFuelByJurisdiction).where(IftaFuelByJurisdiction.ifta_return_id == ifta_return.id)
    )
...
async def scan_fleet_compliance(db: AsyncSession, org_id: uuid.UUID) -> list[dict]:
    """
            ...
            alerts.append(
                {
                    ...
                    "detail": (
                        f"{len(upcoming_items)} item(s) due within 7 days: "
                        f"{', '.join(i.category for i in upcoming_items)}"
                    ),
                    "severity": "info",
                }
            )
